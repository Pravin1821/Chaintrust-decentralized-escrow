...
exports.raiseDispute = async (req, res) => {
    try {
        ...
        if(contract.dispute){
            return res.status(400).json({message: "Dispute already exists for this contract"});
        }
        ...
        if(contract.client.toString() === userId){
            ...
            return res.status(403).json({message: "Unauthorized to raise dispute on this contract"});
        }
        if(!["Funded", "Submitted"].includes(contract.status)){
            return res.status(400).json({message: "Dispute can only be raised on Funded or Submitted contracts"});
        }
...
exports.getAllDisputes = async (req, res) => {
    try{
        const disputes = await Dispute.find().populate('contract').populate('raisedByUser', 'name email').populate('resolvedBy', 'name email');
